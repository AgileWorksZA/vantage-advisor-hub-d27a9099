...
export function TransactionsSection() {
  ...
  const activeTab = tab || "transaction-types";

  const section = adminSections.find((s) => s.id === "transactions");
  const tabs = section?.tabs || [];

  const listType = tabToListType[activeTab] || "transaction_types";
  ...
  return (
    <div className="p-6 space-y-6">
      <Tabs
        value={activeTab}
        onValueChange={(v) => navigate(`/administration/transactions/${v}`)}
      >
        <TabsList>
          {tabs.map((t) => (
        ...
        <div className="mt-6">
          <AdminSectionHeader
            title={tabs.find((t) => t.id === activeTab)?.label || "Transactions"}
