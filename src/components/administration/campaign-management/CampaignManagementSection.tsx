...
export function CampaignManagementSection() {
  ...
  const section = adminSections.find((s) => s.id === "campaign-management");
  const tabs = section?.tabs || [];

  const listType = tabToListType[activeTab] || "campaign_templates";
  ...
  return (
    <div className="p-6 space-y-6">
      <Tabs
        value={activeTab}
        onValueChange={(v) => navigate(`/administration/campaign-management/${v}`)}
      >
        <TabsList>
          {tabs.map((t) => (
        ...
        <div className="mt-6">
          <AdminSectionHeader
            title={tabs.find((t) => t.id === activeTab)?.label || "Campaign Management"}
