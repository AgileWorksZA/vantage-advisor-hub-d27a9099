...
interface NewBusinessWizardDialogProps {
  open: boolean;
...
const NewBusinessWizardDialog = ({ open, onOpenChange, jurisdiction }: NewBusinessWizardDialogProps) => {
      ...
      <DialogContent className="max-w-6xl p-0 gap-0 overflow-hidden [&>button]:hidden">
        <div className="flex min-h-[600px]">
          ...
          <div className="flex-1 p-8">
            <h2 className="text-xl font-bold text-foreground tracking-wide uppercase">New Business –</h2>
                      ...
                      <div className="space-y-3">
                        {products.map((product) => (
                          <div key={product} className="flex items-center space-x-3">
                            <RadioGroupItem value={product} id={`newbiz-${product}`} />
                            <Label htmlFor={`newbiz-${product}`} className="text-sm font-normal cursor-pointer">
                              {product}
                            </Label>
                          </div>
                        ))}
          ...
          <div className="w-64 bg-muted/30 border-l flex flex-col">
            <div className="p-6 border-b">
              <h3 className="text-center font-bold text-sm uppercase tracking-wider text-foreground">New Business</h3>
            </div>
            ...
            <div className="flex-1 p-6 space-y-6">
              ...
              <div>
                ...
                <div className="space-y-2.5">
                  {NEW_BUSINESS_STEPS.capture.map((step, index) => (
...
export default NewBusinessWizardDialog;
