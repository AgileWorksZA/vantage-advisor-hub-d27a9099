...
import { Phone, Search, Smile, Paperclip, Send, Check, CheckCheck, MessageSquare } from "lucide-react";
...
const MessageStatus = ({ status }: { status: string }) => {
  if (status === "read") {
    return <CheckCheck className="w-3.5 h-3.5 text-[hsl(180,70%,45%)]" />;
...
export const ChatPanel = ({ channel, clientId }: ChatPanelProps) => {
  ...
  // client.client format is "Surname, I (FirstName)" - extract name parts
    ...
    <div className="flex-1 flex flex-col bg-background">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <button
              ...
              className="font-medium text-sm hover:text-[hsl(180,70%,45%)] transition-colors"
            >
              {clientName}
            </button>
            <p className="text-xs text-muted-foreground">
              {channelLabel} • Click to view profile
            </p>
          </div>
        </div>
      ...
      <div className="p-3 border-t border-border">
        <div className="flex items-center gap-2">
          ...
          <Button
            size="icon"
            className="h-9 w-9 shrink-0 bg-[hsl(180,70%,45%)] hover:bg-[hsl(180,70%,40%)]"
            onClick={handleSend}
