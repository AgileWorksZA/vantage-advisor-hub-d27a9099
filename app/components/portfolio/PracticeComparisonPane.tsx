...
interface PracticeComparisonPaneProps {
  groupId: string;
...
export default function PracticeComparisonPane({ groupId, currentFunds, currencySymbol }: PracticeComparisonPaneProps) {
  const { selectedRegion } = useRegion();
  const jurisdiction = selectedRegion;

  const currentPortfolio = useMemo(() =>
    currentFunds.map(f => ({ name: f.name, allocation: f.allocation })),
    [currentFunds]
  );
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      if (searchRef.current && !searchRef.current.contains(e.target as Node)) setShowFundDropdown(false);
    };
  ...
  const filteredFunds = useMemo(() => {
    ...
    return availableFunds.filter(f =>
      f.name.toLowerCase().includes(q) || (f.code && f.code.toLowerCase().includes(q)) || (f.fund_manager && f.fund_manager.toLowerCase().includes(q))
    ).slice(0, 20);
  ...
  const evenSplit = (funds: ComparisonFund[]): ComparisonFund[] => {
    ...
    return funds.map((f, i) => ({ ...f, allocation: i === count - 1 ? +(100 - each * (count - 1)).toFixed(1) : each }));
  };
  ...
  const updateAllocation = (fundId: string, value: number) => {
    ...
    setComparisonFunds(prev => prev.map(f => f.fundId === fundId ? { ...f, allocation: value } : f));
  };
  ...
  const autoBalance = () => {
    ...
    if (count === 0) return;
    const each = +(100 / count).toFixed(1);
    setComparisonFunds(prev => prev.map((f, i) => ({ ...f, allocation: i === count - 1 ? +(100 - each * (count - 1)).toFixed(1) : each })));
  ...
  // Computed data
  const currentFees = useMemo(() => generateFees(currentPortfolio, `current-${groupId}-`), [currentPortfolio, groupId]);
  const comparisonFees = useMemo(() => hasComparison ? generateFees(comparisonFunds.map(f => ({ name: f.name, allocation: f.allocation })), `comp-${groupId}-`) : [], [comparisonFunds, hasComparison, groupId]);
  ...
  const perfReturns = useMemo(() => hasComparison ? generatePerformanceReturns(currentPortfolio, comparisonFunds.map(f => ({ name: f.name }))) : [], [currentPortfolio, comparisonFunds, hasComparison]);
  const historicalPerfRaw = useMemo(() => hasComparison ? generateHistoricalPerformance(currentPortfolio, comparisonFunds.map(f => ({ name: f.name }))) : [], [currentPortfolio, comparisonFunds, hasComparison]);
  ...
  const currentHoldings = useMemo(() => generateHoldings(currentPortfolio, `current-top-${groupId}`), [currentPortfolio, groupId]);
  const compHoldings = useMemo(() => hasComparison ? generateHoldings(comparisonFunds, `comp-top-${groupId}`) : [], [comparisonFunds, hasComparison, groupId]);
  const currentAlloc = useMemo(() => generateAssetAllocation(currentPortfolio, `current-${groupId}`), [currentPortfolio, groupId]);
  const compAlloc = useMemo(() => hasComparison ? generateAssetAllocation(comparisonFunds, `comp-${groupId}`) : [], [comparisonFunds, hasComparison, groupId]);

  // Chart options
  const barChartOption = useMemo(() => {
    if (perfReturns.length === 0) return {};
    const labels = perfReturns.map(p => {
      const map: Record<string, string> = { "6M": "6-Month", "1Y": "1-Year", "3Y": "3-Year", "5Y": "5-Year", "7Y": "7-Year", "10Y": "10-Year" };
      return map[p.period] || p.period;
    });
    return {
      tooltip: { trigger: "axis" as const },
      legend: { data: ["Current", "Comparison"], bottom: 0 },
      xAxis: { type: "category" as const, data: labels, axisLabel: { fontSize: 10, rotate: 20 } },
      yAxis: { type: "value" as const, axisLabel: { formatter: "{value}%" } },
      grid: { left: 50, right: 20, top: 20, bottom: 50 },
      series: [
        { name: "Current", type: "bar" as const, data: perfReturns.map(p => p.currentReturn), itemStyle: { color: CURRENT_COLOR }, barGap: "10%" },
        { name: "Comparison", type: "bar" as const, data: perfReturns.map(p => p.comparisonReturn), itemStyle: { color: COMP_COLOR } },
      ],
    };
  }, [perfReturns]);
  ...
  const lineChartOption = useMemo(() => {
    ...
    return {
      ...
      dataZoom: [{ type: "inside" as const }, { type: "slider" as const, bottom: 25, height: 18 }],
      grid: { left: 50, right: 20, top: 20, bottom: 70 },
      series: [
        { name: "Current", type: "line" as const, data: historicalPerf.map(p => p.current), smooth: true, showSymbol: false, lineStyle: { color: CURRENT_LINE, width: 2 }, itemStyle: { color: CURRENT_LINE } },
        { name: "Comparison", type: "line" as const, data: historicalPerf.map(p => p.comparison), smooth: true, showSymbol: false, areaStyle: { opacity: 0.2, color: COMP_COLOR }, lineStyle: { color: COMP_LINE, width: 2 }, itemStyle: { color: COMP_LINE } },
      ],
    };
  }, [historicalPerf]);
  ...
  const holdingsBarOption = (items: { name: string; percentage: number }[], color: string) => ({
    ...
    grid: { left: 100, right: 16, top: 4, bottom: 4 },
    series: [{ type: "bar" as const, data: items.map(h => h.percentage).reverse(), itemStyle: { color, borderRadius: [0, 3, 3, 0] }, barWidth: 14 }],
  });
  ...
  const pieOption = (data: { name: string; value: number }[], title: string) => ({
    ...
    series: [{ type: "pie" as const, radius: ["30%", "60%"], center: ["50%", "55%"], data, label: { formatter: "{b}\n{d}%", fontSize: 9 } }],
  });

  const Placeholder = ({ text }: { text: string }) => (
    <div className="flex items-center justify-center h-full min-h-[120px] text-xs text-muted-foreground italic">{text}</div>
  );

  const FeeTable = ({ fees, title, referenceFees, mode }: { fees: FeeRow[]; title: string; referenceFees?: FeeRow[]; mode?: "current" | "comparison" }) => {
    const weightedAvg = (key: keyof FeeRow) => fees.reduce((s, f) => s + (f[key] as number) * (f.allocation / 100), 0);
    const refWeightedAvg = (key: keyof FeeRow) => referenceFees ? referenceFees.reduce((s, f) => s + (f[key] as number) * (f.allocation / 100), 0) : undefined;
          ...
          <TableBody>
            ...
            <TableRow className="border-t-2">
              ...
              {(["investmentMgmtFee", "adminFee", "advisorFee", "otherFee", "totalCost"] as (keyof FeeRow)[]).map(key => {
                ...
                return <TableCell key={key} className={`text-[10px] py-1 text-right font-bold ${hl(val, refVal)}`}>{val.toFixed(2)}%</TableCell>;
              })}
            </TableRow>
          </TableBody>
        </Table>
    ...
    <div className="flex items-center gap-3">
      {options.map(opt => (
        <label key={opt.value} className="flex items-center gap-1.5 cursor-pointer text-xs">
          <span className={`w-3.5 h-3.5 rounded-full border-2 flex items-center justify-center ${value === opt.value ? "border-teal-600" : "border-muted-foreground/40"}`}>
  ...
  return (
    <div className="space-y-4 pt-2">
      {/* Row 1: Current & Comparison Fund Selection */}
            ...
            <Table>
              ...
              <TableBody>
                {currentFunds.map((f, i) => (
                  <TableRow key={i}>
                    <TableCell className="text-[11px] py-1 font-medium">{f.name}</TableCell>
                    <TableCell className="text-[11px] py-1 text-right">{f.allocation}%</TableCell>
                    <TableCell className="text-[11px] py-1 text-right">{currencySymbol}{f.value.toLocaleString(undefined, { maximumFractionDigits: 0 })}</TableCell>
                  </TableRow>
                ))}
                <TableRow className="border-t-2">
                  ...
                  <TableCell className="text-[11px] py-1 text-right font-bold">{currencySymbol}{currentFunds.reduce((s, f) => s + f.value, 0).toLocaleString(undefined, { maximumFractionDigits: 0 })}</TableCell>
                </TableRow>
              </TableBody>
            </Table>
          </CardContent>
        ...
        <Card>
          <CardHeader className="pb-2 pt-3 px-4">
            <CardTitle className="text-sm text-teal-600 flex items-center justify-between">
              ...
              <Button variant="outline" size="sm" className="h-7 text-xs" onClick={autoBalance} disabled={comparisonFunds.length === 0}>Auto Balance</Button>
            </CardTitle>
          </CardHeader>
          <CardContent className="px-4 pb-3 space-y-2">
            <div ref={searchRef} className="relative">
              <div className="relative">
                ...
                <Input placeholder="Search funds..." className="pl-8 h-7 text-xs" value={fundSearch} onChange={e => { setFundSearch(e.target.value); setShowFundDropdown(true); }} onFocus={() => setShowFundDropdown(true)} />
              </div>
              {showFundDropdown && (
                <div className="absolute top-full left-0 right-0 mt-1 bg-popover border rounded-md shadow-lg z-50 max-h-[200px] overflow-auto">
                  <ScrollArea className="max-h-[200px]">
                    {filteredFunds.length === 0 ? (
                      ...
                      <button key={fund.id} className="w-full text-left px-3 py-1.5 hover:bg-accent/50 transition-colors border-b last:border-b-0" onClick={() => addFund(fund)}>
              ...
              <div className="space-y-1">
                {comparisonFunds.map(fund => (
                  <div key={fund.fundId} className="flex items-center gap-1.5 bg-muted/50 rounded px-2 py-1">
                    <div className="flex-1 min-w-0"><div className="text-[11px] font-medium truncate">{fund.name}</div></div>
                    <Input type="number" min={0} max={100} step={0.1} className="w-[4.5rem] h-6 text-[10px] text-right" value={fund.allocation} onChange={e => updateAllocation(fund.fundId, +e.target.value)} />
                    <span className="text-[10px] text-muted-foreground">%</span>
                    <Button variant="ghost" size="icon" className="h-5 w-5" onClick={() => removeFund(fund.fundId)}><X className="h-3 w-3" /></Button>
                  </div>
                ))}
                <div className="flex justify-end text-[11px]">
                  <span className={`font-medium ${Math.abs(compAllocTotal - 100) > 1 ? "text-destructive" : "text-muted-foreground"}`}>Total: {compAllocTotal.toFixed(1)}%</span>
                </div>
              </div>
            )}
          </CardContent>
      ...
      {/* Row 2: Fee Comparison */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card>
          <CardHeader className="pb-1 pt-3 px-4"><CardTitle className="text-sm text-teal-600">Current Fees & EAC</CardTitle></CardHeader>
          <CardContent className="px-4 pb-3">
        ...
        <Card>
          <CardHeader className="pb-1 pt-3 px-4"><CardTitle className="text-sm text-teal-600">Comparison Fees & EAC</CardTitle></CardHeader>
          <CardContent className="px-4 pb-3">
            {hasComparison ? (
              <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card>
          <CardHeader className="pb-1 pt-3 px-4"><CardTitle className="text-sm text-teal-600">Performance Returns</CardTitle></CardHeader>
          <CardContent className="px-4 pb-3">
            {hasComparison ? <EChartsWrapper option={barChartOption} height={260} /> : <Placeholder text="Add funds to see comparison" />}
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-1 pt-3 px-4">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm text-teal-600">Historical Performance</CardTitle>
              <RadioToggle value={historyFrequency} onChange={v => setHistoryFrequency(v as any)} options={[{ value: "monthly", label: "Monthly" }, { value: "quarterly", label: "Quarterly" }, { value: "annually", label: "Annually" }]} />
            </div>
          </CardHeader>
          <CardContent className="px-4 pb-3">
            {hasComparison ? <EChartsWrapper option={lineChartOption} height={260} /> : <Placeholder text="Add funds to see comparison" />}
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card>
          <CardHeader className="pb-1 pt-3 px-3"><CardTitle className="text-xs text-teal-600">Top 10 Holdings</CardTitle></CardHeader>
          <CardContent className="px-2 pb-2"><EChartsWrapper option={holdingsBarOption(currentHoldings, CURRENT_COLOR)} height={220} /></CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-1 pt-3 px-3"><CardTitle className="text-xs text-teal-600">Top 10 Holdings Comparison</CardTitle></CardHeader>
          <CardContent className="px-2 pb-2">
            {hasComparison ? <EChartsWrapper option={holdingsBarOption(compHoldings, COMP_COLOR)} height={220} /> : <Placeholder text="Add funds to see comparison" />}
          </CardContent>
        </Card>
      </div>

      {/* Row 5: Asset Allocation */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card>
          <CardHeader className="pb-1 pt-3 px-4"><CardTitle className="text-sm text-teal-600">Current Asset Allocation</CardTitle></CardHeader>
        ...
        <Card>
          <CardHeader className="pb-1 pt-3 px-4"><CardTitle className="text-sm text-teal-600">Comparison Asset Allocation</CardTitle></CardHeader>
          <CardContent className="px-4 pb-3 space-y-3">
            {hasComparison ? (
              <>
                <Table>
